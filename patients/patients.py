...
from patients.hyperparamsA_slf_b5 import hyperparams_slf_b5 as hyperparams_slf_b5
from patients.hyperparamsA_slf_b4 import hyperparams_slf_b4 as hyperparams_slf_b4

from patients.hyperparamsA_sup_b5 import hyperparams_sup_b5 as hyperparams_sup_b5
from patients.hyperparamsA_sup_b4 import hyperparams_sup_b4 as hyperparams_sup_b4
...
class IVIM_fit():
    """
    In vivo with the fitting algorithms. 
    """
    ...
    def __init__(self, patient_id, mask_name, method, dataset_name, num_bvals, session_name = '', save_name='default'):
        ...
        if dataset_name == 'dataA':
            ...
            print('Invalid input for dataset_name')
        if not os.path.exists(self.dir_out):
            os.makedirs(self.dir_out)

        # load_data gives these variables values
    ...
    def load_data(self):
        """
        Loads the four-dimentional image data for the patient and its GTV that are of interest.
        """
        ...
        # only use 4 b-values from the patient dataset with 5 b-values, if desired
    ...
    def fit_ivim(self, bounds=([0, 0, 0.005, 0.7], [0.005, 0.7, 0.3, 1.3]), fitS0=True, cutoff=200):
        ...
        if self.method == 'seg':
            Dt_1d, Fp_1d, Dp_1d, S0_1d, rmse_1d = from_SEG_fitting.fit_least_squares_array(self.bvals, dwi_2d_data, mask_1d_data, bounds=bounds, fitS0=fitS0, cutoff=cutoff)
...
class IVIM_dnn_LOO():
    """
    In vivo with the DNN algorithms.
    """

    def __init__(self, patient_id, dataset_name, num_bvals, method, training_method='PT', session_name = '', save_name='default'):
        self.patient_id = patient_id
        self.dataset_name = dataset_name
        self.num_bvals = num_bvals
        self.method = f'dnn_{method}'
        ...
        if self.dataset_name == 'dataA':
            if self.num_bvals == 5:
                self.bvals = np.array([0, 50, 100, 200, 800])
                arg = hyperparams_slf_b5()
            elif self.num_bvals == 4:
                self.bvals = np.array([0, 50, 100, 800])
                arg = hyperparams_slf_b4()
            else:
                print('Invalid number of b-values for dataA')
    ...
    def preparations(self, patient_ids, mode):
        """
        Depending on the mode, the data is prepared for either trianing or prediction. 
        More spcifically, if mode is set to 'training', 'patient_ids' should contain a list of several patient IDs. DWI 
        data from the patients specified in 'patient_ids' is then being preprocessed and prepared to be used in the 
        fuction 'train_ivim'. If mode is set to 'prediction', 'patient_ids' should contain a list consisteing of only 
        one patient ID. The DWI data, along with the binary GTVn data, of this patient is being preprocessed and 
        prepared to be used in the function 'predict_ivim'.
        """

        if mode == 'training':
            preprocessed_dwis_2d_data = []      # to be returned

        # load dwi data
        for patient_id in patient_ids:
            if self.dataset_name == 'dataA':
                print(glob.glob(f'../dataA/preprocessed/{patient_id}/dwi_4d/*.nii'))
            ...
            # load mask names for patient with patient id 'patient_id'
            mask_names_patient = np.load(f'../{self.dataset_name}/preprocessed/{patient_id}/mask_names.npy')

            # load binary mask data for patient with patient id 'patient_id'
            mask_data_patient = []
            for mask_name in mask_names_patient:
                ...
                # make folder for predicted parameter maps of GTV with name 'mask_name'
                if mode == 'prediction':
            ...
            # select valid data to train (and predict) on
            S0 = np.squeeze(dwi_2d_data[:, self.bvals == 0])
            if self.training_method == 'GTVn':          # only GTVns
                valid_idx_set = np.array(combined_mask_1d_data, dtype=bool)
            elif self.training_method == 'PT':          # all voxels within patients after filtering out background voxels but with voxels from GTVns included
            ...
            # add valid voxels from patient with patient id 'patient_id' to array containing valid voxels from all patients given in 'patient_ids'
            if mode == 'training':
    ...
    def train_ivim(self, patient_ids):
        """
        Trains selfsupervised DNN (DNN_SSL) on data from patients with patient IDs listed in 'patient_ids'.
        """
    ...
    def predict_ivim(self, patient_id, net):
        """
        The DNN specified by 'net' predicts the IVIM parameter, S0 and signal-RMSE maps of patient with patient ID 
        spcified by 'patient_id'.
        """
        ...
        # define names of parameters to be computed
        param_names = ['Dt', 'fp', 'Dp', 'S0', 'rmse']
        ...
        param_medians = np.zeros((len(mask_names_patient), len(param_names)))
        for i in range(len(param_names)):
            img = np.zeros([self.width * self.height * num_slices])
            img[valid_idx_set] = params_pred[i][0:sum(valid_idx_set)]
            img = np.reshape(img, [self.width, self.height, num_slices])
            nib.save(nib.Nifti1Image(img, dwi_4d_obj.affine, dwi_4d_obj.header), f'{self.dir_out}/{param_names[i]}.nii.gz'),

            for j in range(len(mask_names_patient)):                    
                param_medians[j][i] = np.median(img[mask_data_patient[j]!= 0])
            
        for k in range(len(mask_names_patient)):
            dir_out_pred = os.path.join(self.dir_out, f'{mask_names_patient[k]}')
            if not os.path.exists(dir_out_pred):
                os.makedirs(dir_out_pred)
            np.savetxt(os.path.join(dir_out_pred, f'param_medians.txt'), param_medians[k])
...
Comment: 
In vivo with the DNN algorithms.  
This function trains on only a specified patient and predicts its ivim parameters. In other words, it trains directly on 
the data of interest. Thus, it can be concidered a fancy fitting algorithm that makes use of neural networks. 
This class is NOT used in the thesis. This class may contain errors. However, it has been added as a help or as a template
if you want to try this approach. Small adjustments to the source code may be necessary.
"""
    ...
    def __init__(self, patient_id, mask_names, learning, dataset_name, num_bvals, training_method, session_name = '', save_name ='default'):
        ...
        if dataset_name == 'dataA':
            if self.learning == 'slf':
                ...
                if self.num_bvals == 5:
                    self.bvals = np.array([0, 50, 100, 200, 800])
                    arg = hyperparams_sup_b5()
                elif self.num_bvals == 4:
                    self.bvals = np.array([0, 50, 100, 800])
                    arg = hyperparams_sup_b4()
                else:
                    print('Invalid number of b-values for dataA')
            else:
